...
module.exports = {
    renderMonkeys(link, doLog = true) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            mkdirSync(`./tmp/${uuid}`);
            let time = new Date();
            const [impath, extension] = yield downloadImage(link, `./tmp/${uuid}/monkeyDownload`);
            if (doLog) {
                console.log(`Monkeys took ${(new Date() - time) / 1000.0}s to download input.`);
                time = new Date();
            }
            yield doFfmpeg(["-i", impath + "." + extension, "-frames:v", "1", "-y", `./tmp/${uuid}/monkeyStuff.png`]);
            if (doLog) {
...
`;
            yield renderBlend("./extras/monkeys.blend", ["-o", `//../tmp/${uuid}/monkeyPic####`, "-f", "0"], pythonics);
            if (doLog) {
    ...
    renderBarrel(link, doLog = true) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let time = new Date();
            const [impath, extension] = yield downloadImage(link, `./tmp/${uuid}/barrelDownload`);
            if (doLog) {
                console.log(`Barrel took ${(new Date() - time) / 1000.0}s to download input.`);
                time = new Date();
            }
            yield toGoodVideo(impath + "." + extension, 25, `${uuid}/barrelPicture`, 2.5);
            if (doLog) {
...
`;
            yield renderBlend("./extras/barrel.blend", ["-o", `//../tmp/${uuid}/barrelResult.mkv`, "-a"], pythonics);
            if (doLog) {
                console.log(`Barrel took ${(new Date() - time) / 1000.0}s to render.`);
                time = new Date();
            }
            yield doFfmpeg(["-i", `./tmp/${uuid}/barrelResult.mkv`, "-lavfi", "[0:v]palettegen[pal]; [0:v][pal]paletteuse", "-y", `./tmp/${uuid}/barrelRoll.gif`]);
            if (doLog)
    ...
    renderSnap(link, renderParams, messageToReply = undefined, replyPayload = { content: "Working on snap..." }, doLog = true) {
        return __awaiter(this, void 0, void 0, function* () {
            const uuid = Math.random().toString(36).substring(2, 7) + Math.random().toString(36).substring(2, 7);
            mkdirSync(`./tmp/${uuid}`);
            if (doLog)
            ...
            let time = new Date();
            const [impath, extension] = yield downloadImage(link, `./tmp/${uuid}/snapDownload`);
            if (doLog) {
                console.log(`Snap took ${(new Date() - time) / 1000.0}s to download input.`);
                time = new Date();
            }
            let resolution = yield getResolution(impath + "." + extension);
            resolution = evenify(resolution);
            yield toGoodVideo(impath + "." + extension, 30, `${uuid}/snapvid`, 7, resolution[0], resolution[1]);
            if (doLog) {
            ...
            let angle = 0;
            if (renderParams.angle)
                angle = renderParams.angle;
            let shape = "Tile";
            if (renderParams.shape)
                shape = renderParams.shape;
            let distance = 3.0 * 0.01;
            if (renderParams.distance)
                distance = renderParams.distance;
            let size = 1.0;
            if (renderParams.size)
                size = renderParams.size;
            let color = "36393F";
            if (renderParams.color)
                color = renderParams.color;
            if (messageToReply) {
...
`;
            yield renderBlend("extras/snapped.blend", ["-o", `//../tmp/${uuid}/snaprender.mp4`, "-a"], pythonics);
            if (doLog) {
                console.log(`Snap took ${(new Date() - time) / 1000.0}s to render.`);
                time = new Date();
            }
            yield doFfmpeg(["-i", `./tmp/${uuid}/snaprender.mp4`, "-i", `./tmp/${uuid}/snapvid.mkv`, "-map", "0:v", "-map", "1:a?", "-af", "afade=t=out:st=4:d=3.5", "-y", `./tmp/${uuid}/snapped.mp4`]);
            if (doLog) {
    ...
    renderWelcome(link, renderParams, doLog = true) {
        return __awaiter(this, void 0, void 0, function* () {
            const uuid = Math.random().toString(36).substring(2, 7) + Math.random().toString(36).substring(2, 7);
            mkdirSync(`./tmp/${uuid}`);
            if (doLog)
            ...
            let time = new Date();
            const [, extension] = yield downloadImage(link, `./tmp/${uuid}/welcomeDownload`);
            if (doLog) {
                console.log(`Welcome took ${(new Date() - time) / 1000.0}s to download input.`);
                time = new Date();
            }
            let userMention = "You forgot to add a mention!!!";
            if (renderParams.userMention)
                userMention = renderParams.userMention;
            let hideText = false;
            if (renderParams.hideText)
                hideText = true;
            const python = `
import bpy
...
`;
            ...
            yield renderBlend("./extras/welcome.blend", ["-S", renderParams.scene, "-o", `//../tmp/${uuid}/welcomeResult####`, "-f", "0"], python);
            if (doLog) {
