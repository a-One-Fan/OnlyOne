...
const MENTION_LINK_GUILD = "https://guild.mentionPfp.com/pfp.png";
const AUTHOR_LINK = "https://www.authorpfp.com/pfp.png";
const AUTHOR_LINK_GUILD = "https://guild.authorpfp.com/pfp.png";
const REPLIED_AUTHOR_LINK = "https://www.repliedpfp.com/pfp.png";
const REPLIED_AUTHOR_LINK_GUILD = "https://guild.repliedpfp.com/pfp.png";
const ONLYONE_LINK = "https://o.ne/pfp.png";
const ONLYONE_LINK_GUILD = "https://o.ne/pfp_guild.png";
...
const USER_LINK = "https://www.who.com/pfp.png";
const GUILD_USER_LINK = "https://guild.who.com/pfp.png";
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const fakeMessage = { content: "",
                client: { db: { findOne(query) {
                            if (query && query.where && query.where.userID == 1)
                                return fakeRow;
                            return undefined;
                        } } },
                author: { id: 1 },
            ...
            for (const test of tests) {
                ...
                if (test.parseType)
                    fakeRow.parseType = test.parseType;
                if (test.errorType)
                    fakeRow.errorType = test.errorType;
                if (test.func)
                    func_to_test = test.func;
                if (!test.in)
                    continue;
                let res;
                if (func_to_test == SIMULATE_MESSAGE) {
                    ...
                    const func = () => __awaiter(this, void 0, void 0, function* () {
                        ...
                        try {
                            catchres = yield func_to_test(...test.in);
                        }
                        catch (err) {
                            return err;
                        }
            ...
            for (const res of results) {
                ...
                if (err) {
                    ...
                    let isEqual = true;
                    if (res.test.out)
                        isEqual = isEqual && checkSingularOut(res.test.out);
                    if (res.test.notOut)
                        isEqual = isEqual && (!checkSingularOut(res.test.notOut));
            ...
            for (const test of results)
                if (test.result && test.result.cleanup)
                    rm(test.result.cleanup, { recursive: true, force: true }, (err) => { if (err)
