...
module.exports = {
    ...
    getConverter(oldunit, newunit) {
        if (!oldunit.subtype || !newunit.subtype)
            return false;
        if (oldunit.subtype == newunit.subtype)
            return false;
    ...
    convertUnit(val, newunit) {
        if (!val.chunkType == module.exports.chunkTypes.number)
            throw Error(`Trying to convert not-number ${val} to units ${newunit}`);
        if (val.unit == newunit)
            return val;
        ...
        if (numRegex) {
            ...
            for (let o in module.exports.operators) {
                ...
                if (module.exports.find(o.names, chunk) > -1) {
                    // TODO: see line 120
                    if (o.args == 1)
                        return { chunkType: module.exports.chunkTypes.operator, args: o.args, op: module.exports.makeOp1Arg(o.op) };
                    else
                        return { chunkType: module.exports.chunkTypes.operator, args: o.args, op: module.exports.makeOp2Arg(o.op) };
                }
            }
            if (chunk == "(")
                return { chunkType: module.exports.chunkTypes.openingBracket };
            if (chunk == ")")
                return { chunkType: module.exports.chunkTypes.closingBracket };
            try {
                ...
                for (const cur in currencies) {
                    if (chunk.toUpperCase() == cur || (module.exports.find(currencySynonyms[cur], chunk) > -1)) {
                        let _names = [cur];
                        if (currencySynonyms[cur])
                            _names = _names.concat(currencySynonyms[cur]);
    ...
    stringifyChunk(chunk) {
        if (chunk.chunkType == module.exports.chunkTypes.openingBracket)
            return "(";
        if (chunk.chunkType == module.exports.chunkTypes.closingBracket)
            return ")";
        if (chunk.chunkType == module.exports.chunkTypes.operator)
            return "op";
        if (chunk.chunkType == module.exports.chunkTypes.number)
    ...
    convertInfix(translated) {
        ...
        for (const c of translated) {
            ...
            if (c.priority)
                tempPriorityChunk = c.priority;
            let tempPriorityStack = 0;
            if (conversionStack.at(-1).priority)
                tempPriorityStack = conversionStack.at(-1).priority;
